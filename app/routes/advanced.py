...
def index():
    ...
    recent=RecoveryCallLog.query.order_by(RecoveryCallLog.created_at.desc()).limit(20).all()
    high_value=LapsedPolicy.query.filter(LapsedPolicy.premium_due>0).order_by(LapsedPolicy.premium_due.desc()).limit(20).all()
